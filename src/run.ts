        ...
        for (const line of lines) {
            ...
            if (match) {
                ...
                if (unpackedSizeBytes >= thresholdBytes) {
                    const largeFileEntry: LargeFileDetail = {
                        // Use the interface
                        path: filePath,
                        blobSha: blobSha,
                    ...
                    core.debug(
                        `Searching for commits related to blob ${blobSha} (path hint from report: ${filePath})`,
                    );
                    let commitLogOutput = "";
                    try {
                        await exec.exec(
                            ...
                            [
                                "log",
                                "--all", // Search across all history
                                "--pretty=format:%H", // Output full commit SHAs
                                `--find-object=${blobSha}`, // Find commits referencing this specific blob
                            ],
                            {
                                ...
                                silent: true, // Keep CI logs cleaner by default
                                ignoreReturnCode: true, // The command might find no commits (not an error for us)
                                // or fail in very rare edge cases with the object.
                            },
                        );

                        const commitShas = commitLogOutput
                            .split("\n")
                            .filter((sha) => sha.trim().length > 0);
                        if (commitShas.length > 0) {
                            largeFileEntry.commits = commitShas.map((sha) =>
                                sha.substring(0, 7),
                            ); // Store short SHAs
                            core.info(
                                `Blob ${blobSha} (path hint: ${filePath}) found in commits: ${largeFileEntry.commits.join(", ")}`,
                            );
                        } else {
                            core.info(
                                `Blob ${blobSha} (path hint: ${filePath}) - no specific introducing commits found via --find-object (or max-count hit early).`,
                            );
                        }
                    } catch (logError: any) {
                        core.warning(
                            `Could not retrieve commit history for blob ${blobSha} using --find-object: ${logError.message}`,
                        );
                    }
            ...
            core.setOutput(
                "large-files-list",
                JSON.stringify(detectedLargeFiles),
            );
            ...
            if (detectedLargeFiles.length > 0) {
                ...
                detectedLargeFiles.forEach((file) => {
                    ...
                    if (file.commits && file.commits.length > 0) {
                        errorMessage += `  Referenced in commits: ${file.commits.join(", ")}\n`;
                    }
                });
                ...
                if (
                    ...
                    try {
                        ...
                        core.warning(
                            `Failed to create PR comment: ${commentError.message}`,
                        );
                    }
                }
            } else {
